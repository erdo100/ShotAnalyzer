dirs = dir("E:\DaVinciResolveProjects\20221208_OffsetSystem\20230114_OBS\20230114*.bbt");
results = [];
results{1} = 'Filename';
results{2} = 'Offset';
results{3} = 'B2 Pos X';
...
for fi = 1:length(dirs)
    ...
    B1vel = sqrt((data(i1,2)-data(1,2))^2 + (data(i1,3)-data(1,3))^2)/(data(i1,1)-data(1,1));
    disp(['B1vel=',num2str(B1vel),'m/s'])
    
    %% Calculate direction B1
    B1angle = atan2(data(i1,3)-data(1,3), data(i1,2)-data(1,2))/pi*180;
    disp(['B1angle=',num2str(B1angle),'°'])
    
    %% Calculate B2 Position
    B2pos = [mean(data(1:i1,4)), mean(data(1:i1,5))];
    
    %% Calculate Hit Fraction
    dirvec = [data(1,2) data(1,3) 0 ] - [data(i1-3,2) data(i1-3,3) 0];
    hitThickness = 1 - norm(cross([B2pos 0]-[data(1,2) data(1,3) 0], dirvec))/norm(dirvec) / 61.5;
    
    %% Calculate B1 1st Cushion hit
    ii = i1-1+find(data(i1:end,3) > 1350,1,'first');
    ...
    ii = ib1_c1L1-1+find(data(ib1_c1L1:end,2) > 2780,1,'first');
    [~, ib1_c2S2] = max(data(ii:ii+10,2));
    ib1_c2S2 = ii-1+ib1_c2S2;
    % Check Long cushion 3 hit 2
    ii = ib1_c1L1-1+find(data(ib1_c1L1:end,3) < 60,1,'first');
...
writecell(results,['E:\DaVinciResolveProjects\20221208_OffsetSystem\ShotAnalyzer_Data.xlsx'])