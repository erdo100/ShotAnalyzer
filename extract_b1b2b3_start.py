...
# Needs the main data structure SA (containing 'Shot' list and 'Data' DataFrame)
    ...
    for si, current_shot_id in enumerate(SA['Data']['ShotID']):
        ...
        try:
            ...
            if SA['Data'].iloc[si]['Interpreted'] == 0:
                ...
                if err_info['code'] is not None:
                    SA['Data'].loc[SA['Data'].index[si], 'ErrorID'] = err_info['code']
                    SA['Data'].loc[SA['Data'].index[si], 'ErrorText'] = err_info['text']
             ...
             print(f"Error: Index {si} out of bounds for SA['Data'] or SA['Shot'] during B1B2B3.")
             # Optionally mark this as an error in the table
             SA['Data'].loc[SA['Data'].index[si], 'ErrorID'] = 98 # Custom error code
             SA['Data'].loc[SA['Data'].index[si], 'ErrorText'] = 'Internal index error during B1B2B3 processing'
             SA['Data'].loc[SA['Data'].index[si], 'Selected'] = True
             selected_count +=1
        except Exception as e:
             print(f"Error processing shot index {si} for B1B2B3: {e}")
             SA['Data'].loc[SA['Data'].index[si], 'ErrorID'] = 97 # Custom error code
